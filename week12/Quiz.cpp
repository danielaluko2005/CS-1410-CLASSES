...
int Reviews::GetMaxRating() const
{ 
    int maximum;
    unsigned int i;

    maximum = reviewList.at(0).GetRating();
    for (i = 0; i < reviewList.size(); ++i)
    {
        if (reviewList.at(i).GetRating() > maximum)
        {
            maximum = reviewList.at(i).GetRating();
        }
    }
    return maximum;
}
...
int main()
{
    ...
    cout<<"Maximum rating: "<<allReviews.GetMaxRating()<<endl;

    // Output all comments for given rating
    cout << endl
         << "Type rating. To end: -1" << endl;
    cin >> currRating;
    while (currRating != -1)
    {
